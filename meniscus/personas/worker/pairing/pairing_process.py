...
import json
import platform
from meniscus.api.utils.sys_assist import get_lan_ip, get_sys_mem_total_MB, \
    get_disk_size_GB, get_cpu_core_count
...
class PairingProcess(object):
    def __init__(self, api_secret, coordinator_uri, personality):
        ...
        self.process = Process(
            target=self.pair_with_coordinator, kwargs=self._kwargs)

    def run(self):
        self.process.start()

    def pair_with_coordinator(self, api_secret, coordinator_uri, personality):
        ...
        registration = {
            ...
            "system_info": {
                ...
                "memory_mb": get_sys_mem_total_MB(),
